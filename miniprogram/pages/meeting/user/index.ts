...
Page({
  ...
  data: {
    formItem: [
      ...
      { label: '用车开始时间', name: 'beginTime', value: '', start: formatTime(new Date()), type: 'picker', mode: 'date', required: { message: '请选择用车开始时间' } },
      { label: '用车结束时间', name: 'endTime', value: '', start: formatTime(new Date()), type: 'picker', mode: 'date',  required: { message: '请选择用车结束时间' }},
      { label: '用车大概行程',  name: 'travel', value: '', type: 'input', required: { message: '请输入用车大概行程' } },
      { label: '预约车型',  name: 'vehicleModel', value: '', type: 'input', required: { message: '请输入预约车型' } },
      { label: '用车数量',  name: 'vehicleCount', inputType: 'number', value: '', type: 'input', required: { message: '请输入用车数量' } },
      { label: '备注',  name: 'remark', value: '', type: 'input'},
    ],
    loading: false
  },
  onLoad() {
